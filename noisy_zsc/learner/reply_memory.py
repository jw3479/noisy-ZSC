...
class RolloutBuffer():
    ...
    def __init__(self, mem_size, input_shape, episode_length):
        ...
        self.state_memory = np.zeros((self.mem_size, self.episode_length, self.input_shape), dtype=np.float32)
        self.new_state_memory = np.zeros((self.mem_size, self.episode_length, self.input_shape), dtype=np.float32)
        self.action_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.int64)
        self.reward_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.float32)
        self.terminal_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.bool8)
        self.log_probs_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.float32)

    def clear_memory(self):
        # clearn memory by filling all with zeros
        self.state_memory = np.zeros((self.mem_size, self.episode_length, self.input_shape), dtype=np.float32)
        self.new_state_memory = np.zeros((self.mem_size, self.episode_length, self.input_shape), dtype=np.float32)
        self.action_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.int64)
        self.reward_memory = np.zeros((self.mem_size, self.episode_length), dtype=np.float32)
    ...
    def store_transition(self, state, actions, reward, state_, done, log_probs):
        mem_index = self.mem_cnt % self.mem_size
        traj_index = self.trajectory_cnt % self.episode_length
        if mem_index == 0:
            # clear memory if filling out a new batch of rollouts
            
            self.clear_memory()            
        self.state_memory[mem_index, traj_index] = state
        self.action_memory[mem_index, traj_index] = actions
        self.reward_memory[mem_index, traj_index] = reward
        self.new_state_memory[mem_index, traj_index] = state_
        self.terminal_memory[mem_index, traj_index] = done
